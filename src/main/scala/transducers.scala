...
object TransducerTrait {
  ...
  trait Transducer[A, B] {
    ...
    def o[C](t2: Transducer[C, A]): Transducer[C, B] = comp(t2)

    def filter(pred:A => Boolean):Transducer[A,B] = this.comp(TransducerTrait.filter(pred))

    def map[C](f:A=> C): Transducer[C, B] = this.comp(TransducerTrait.map(f))
...
object TransducerExistential {
  ...
  def map3[A,B,R](f : A => B) : Transducer3[B,A,R] = { rf : ReducingFn[B,R] =>
    (r : R  ,a:A) => rf(r,f(a))}
  def map[A,B] = map3[A,B,R forSome {type R}]
...
object TransApp extends App {
  ...
  /*
  {
    ...
    println(List("1", "2", "3").foldLeft[Int](0)(t_parse { (i: Int, j: Int) => i + j})
